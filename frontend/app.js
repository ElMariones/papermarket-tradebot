...
function clearView() {
  ...
  clearChat();  // the chat is grounded in ONE bot's log — new bot, new chat
}
...
// ---- TABS ----
const PANES = { trades: "tradesPane", decisions: "decisionsPane",
                reports: "reportsPane", ask: "askPane" };
document.querySelectorAll(".tab").forEach((t) =>
  t.onclick = () => {
    ...
    if (t.dataset.tab === "ask") initAsk();
  });

// ---- ASK THE BOT (local MLX RAG over this bot's own log) ----
// Read-only chat grounded in the selected bot's reasoning log / trade
// history, generated by a small LLM running fully on-device (Apple MLX).
// Open to spectators. Unavailable on non-Apple deployments — the tab stays
// visible but explains itself instead of erroring.
let MLX_STATUS = null;   // null = not yet checked
let ASK_BUSY = false;

async function initAsk() {
  if (MLX_STATUS !== null) return;
  try {
    MLX_STATUS = await (await fetch("/api/mlx-status")).json();
  } catch (e) {
    MLX_STATUS = { available: false, reason: "Could not reach the server." };
  }
  if (!MLX_STATUS.available) {
    $("chatForm").classList.add("hidden");
    const d = $("chatDisabled");
    d.classList.remove("hidden");
    d.textContent = "Ask the Bot requires running locally on an Apple " +
      "Silicon Mac. This deployment doesn't support it.";
    $("chatEmpty").textContent = "The rest of the dashboard works exactly " +
      "the same — this one tab needs the local MLX model.";
  }
}
...
function chatSources(afterEl, sources) {
  if (!sources || !sources.length) return;
  const det = document.createElement("details");
  det.className = "chat-src";
  det.innerHTML = `<summary>${sources.length} log entr${sources.length === 1 ? "y" : "ies"} cited</summary>` +
    sources.map((s) => `<div class="chat-src-item">
      <span class="chat-src-meta">${esc(s.source_type)} · ${ts(s.timestamp)} · match ${(s.score * 100).toFixed(0)}%</span>
      <pre>${esc(s.text)}</pre></div>`).join("");
  afterEl.insertAdjacentElement("afterend", det);
}

$("chatForm").onsubmit = async (ev) => {
  ev.preventDefault();
  const q = $("chatInput").value.trim();
  if (!q || ASK_BUSY || !MLX_STATUS || !MLX_STATUS.available) return;
  $("chatInput").value = "";
  const hideEmpty = $("chatEmpty"); if (hideEmpty) hideEmpty.classList.add("hidden");
  chatBubble("user", q);
  const pending = chatBubble("bot pending", MLX_STATUS.loaded
    ? "thinking…"
    : "waking up the local model… (first question loads it, ~30s)");
  ASK_BUSY = true; $("chatSend").disabled = true;
  const epoch = VIEW_EPOCH;
  try {
    const r = await api("/api/ask", "POST", { question: q });
    if (stale(epoch)) return;  // switched bots — chat was cleared
    pending.classList.remove("pending");
    if (r.error) {
      pending.textContent = "✗ " + r.error;
    } else {
      pending.textContent = r.answer;
      chatSources(pending, r.sources);
      MLX_STATUS.loaded = true;
    }
  } catch (e) {
    if (!stale(epoch)) pending.textContent = "✗ Request failed — try again.";
  } finally {
    ASK_BUSY = false; $("chatSend").disabled = false;
  }
};

// the chat is about ONE bot's log — switching bots starts a fresh scrollback
function clearChat() {
  const log = $("chatLog");
  if (!log) return;
  log.querySelectorAll(".chat-msg,.chat-src").forEach((el) => el.remove());
  const empty = $("chatEmpty");
  if (empty && MLX_STATUS && MLX_STATUS.available) empty.classList.remove("hidden");
}
